...
export const SpaceExpensesMain = ({
		...
		let rows = list.filter((tx) => {
			...
			if (statusFilter !== "all") {
				...
				if (statusFilter === "other") {
					...
					if (tone === "needs_review" || tone === "cancelled") {
						return false;
					}
				} else if (statusFilter === "needs_review") {
					if (tone !== "needs_review") return false;
	...
	return (
		<WorkspaceListingPage
			description={
				<>
					Saved expense records in {spaceLabel}. New captures and candidate
					review now live in Captures.
				</>
			}
			stats={
				<>
					<WorkspaceSummaryChip label="Records" value={listStats.total} />
					<WorkspaceSummaryChip
						accent="positive"
						label="Recorded"
						value={listStats.recorded}
					/>
					<WorkspaceSummaryChip
						label="Line items"
						value={listStats.lineItems}
					/>
					<WorkspaceSummaryChip
						label="Linked captures"
						value={listStats.linkedCaptures}
					/>
					{listStats.needsReview > 0 ? (
						<WorkspaceSummaryChip
							accent="attention"
							label="Needs review"
		...
		>
					...
					<>
						...
						<label className="flex cursor-pointer items-center gap-2 pb-0.5">
							<input
								...
								type="checkbox"
							/>
							<span className="text-sm text-foreground/85">Recurring only</span>
						</label>
						<button
				...
				sort={
					<label className="flex items-center gap-2">
						...
						<select
							...
							className={`${workspaceControlClass} min-w-[10.5rem]`}
			...
			>
				<select
					...
					className={`${workspaceControlClass} min-w-[7.5rem]`}
				...
				<select
					...
					className={`${workspaceControlClass} min-w-[6.5rem]`}
			...
			<WorkspaceListBody error={listError}>
				{actionError ? (
				...
				{!listLoading && transactions && transactions.length === 0 ? (
					<div className="rounded-2xl border border-[rgba(120,100,80,0.14)] bg-[rgba(255,252,246,0.72)] px-4 py-5 shadow-sm">
						...
						<p className="mt-1 max-w-xl text-sm leading-relaxed text-muted-foreground">
							Add an expense or review a capture. Candidate work stays in
							Captures until it becomes a saved expense record.
						</p>
						<div className="mt-4 flex flex-wrap gap-2">
							...
							{spaceId != null ? (
								<Link
									className="inline-flex h-9 items-center rounded-lg border border-[rgba(120,100,80,0.2)] bg-white/80 px-3 text-sm font-medium text-foreground/85 shadow-sm transition hover:bg-white"
					...
					renderItem={(tx) => {
						...
						return (
							<WorkspaceEntityCard
								...
								selected={isSelected}
								summary={
									<div className="flex w-full cursor-pointer gap-3 text-left sm:gap-4">
										<EntityIcon
								...
								tone={isNeedsReview ? "attention" : "default"}
								footer={
									<div className="flex items-center justify-between gap-2">
										<div className="flex flex-wrap items-center gap-2">
											...
											{isNeedsReview && sourceCaptureHref ? (
												<span className="inline-flex h-9 items-center rounded-lg border border-[rgba(181,131,52,0.22)] bg-[rgba(255,247,225,0.74)] px-3 text-sm font-semibold text-[#73501b]">
													Review moved to Captures
												</span>
											) : null}
							...
							>
								{itemRows.length ? (
									<div className="rounded-xl border border-[rgba(120,100,80,0.1)] bg-white/60 px-3 py-2.5">
					...
					>
						...
						<div className="px-5 py-4">
							...
							<div className="mt-4 rounded-xl border border-[rgba(130,70,70,0.18)] bg-[rgba(130,70,70,0.06)] px-3 py-2 text-xs leading-relaxed text-[rgba(100,48,48,0.95)]">
								...
								capture, remove review data from Captures so review context
								stays in one place.
							</div>
