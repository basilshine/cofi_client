...
export const SpaceOverviewPage = () => {
	...
	useEffect(() => {
		...
		void (async () => {
			try {
				const [res, mem, participantRes, captureRes, paymentLinksRes] =
					await Promise.all([
						...
						apiClient.spaces
							...
							.catch(() => null),
						apiClient.paymentLinks.list(numericSpaceId).catch(() => null),
					]);
				if (!cancelled) {
	...
	const spacePosition = useMemo(() => {
		const currentUserParticipantIds = new Set(
			(participants ?? [])
				.filter((participant) =>
					participantBelongsToUser(participant, user?.id),
				)
				.map((participant) => Number(participant.id)),
		);
		const visiblePaymentObligations = new Map<
			string,
			PaymentLinkObligationRef
		>();
		for (const link of paymentLinks.filter(
		...
		const totals = Array.from(visiblePaymentObligations.values()).reduce(
			(acc, obligation) => {
				if (currentUserParticipantIds.has(obligation.payer_participant_id)) {
					acc.youOwe += obligation.amount;
				}
				if (
					currentUserParticipantIds.has(obligation.recipient_participant_id)
				) {
		...
		return {
			...totals,
			net: totals.youAreOwed - totals.youOwe,
		};
	}, [participants, paymentLinks, user?.id]);
	...
	const captureReviewCount = pendingCapturePackets.length;
	const balancesUnsettled = captureReviewCount > 0;
	const spaceDisplayName = space?.name?.trim() || "this space";
	const captureBreakdown = [
		{
			count: capturePacketCounts.expenses,
			label: "expense",
		},
	...
	const recentActivityItems = useMemo(() => {
		...
		return packetItems
			...
			.slice(0, 6);
	}, [capturePackets, space?.name, sidStr, spaceDisplayName]);
						...
						<div className="space-y-0 px-6 py-4">
							<div className="flex items-baseline justify-between gap-3 border-b border-[rgba(105,135,112,0.1)] py-2.5 first:pt-0">
								...
								<span className="text-sm font-medium tabular-nums text-foreground/68">
									{isLoading ? "…" : formatMoney(spacePosition.youOwe)}
								</span>
							</div>
							<div className="flex items-baseline justify-between gap-3 border-b border-[rgba(105,135,112,0.1)] py-2.5">
								...
								<span className="text-sm font-medium tabular-nums text-foreground/68">
									{isLoading ? "…" : formatMoney(spacePosition.youAreOwed)}
								</span>
							</div>
							<div
								className={[
							...
							<p className="pt-3 text-sm leading-relaxed text-muted-foreground">
								Space balances sync from active split-backed payment links.
							</p>
						</div>
					</section>
									...
									{captureReviewCount === 0 ? (
										...
										<>
											...
											<span className="font-semibold text-foreground/90">
												before their expense, split, and benefit candidates
												become records.
											</span>
										</>
									)}
